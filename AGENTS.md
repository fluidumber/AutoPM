...
~/.productflow/
  ...
  products/<slug>/
    ...
    assets/
      ...
      YYYY-MM-DD-<robot>-output.md  Codex's generated analysis text (saved via save-robot-output)
      pdd/
...
const analysis = {
  ...
  _claudeInstructions: {
    ...
    requiredSections: {
      sectionName: {
        instructions: "What Codex must produce here",
...
```js
{
  ...
  phase1Outputs: {          // Codex's saved analysis text per robot
    scout: "markdown text",
...
After each robot run, Codex must call `save-robot-output` with the full markdown
...
structure. The output file contains Codex's actual generated analysis text.
...
`result._improvementHints` before passing to Codex.
