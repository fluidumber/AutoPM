        ...
        {entries.map(([k, meta]) => {
          ...
          return (
            <tr key={k} onClick={() => artifact ? onArtifact(artifact) : onFreshness(k, run, meta)}>
              ...
              <td className="mono" style={{ color: "var(--text-2)" }}>{run.lastRun ? PF.relativeTime(run.lastRun) : "—"}</td>
...
function ArtifactDetailPane({ artifact, product, onCopied }) {
  ...
  return (
    <div className="detail-inner">
      ...
      <div className="detail-body" style={(artifact.type === "html" || window.EPIC_VIEWERS?.[artifact.robot]) ? { padding: 0, height: "calc(100vh - 200px)", overflow: "hidden" } : {}}>
