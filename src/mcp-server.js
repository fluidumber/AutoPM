...
import { ProcessAdvisor } from "../brain/process-advisor.js";
...
const notificationService = new NotificationService(teamLeader.database);
...
IMPORTANT: After showing the user this robot's output, ask them to rate it 1-5 and suggest improvements. Then call the 'feedback' tool before running the next robot.`,
    {
        ...
        epicId: z
            .string()
            .optional()
            .describe("Optional Epic ID for scoped Phase 2 runs."),
        featureId: z
            .string()
            .optional()
...
If the session is tied to a product, the rating is also appended to the robot's asset file on disk.`,
    ...
    async ({ analysisId, robotName, rating, notes, analysisMarkdown, bypassReason }) => {
        ...
        teamLeader.database.ready.then(() =>
            processAdvisor.analyzeIfThresholdMet(robotName).catch(err =>
                console.error("ProcessAdvisor error:", err.message)
            )
        );
        ...
        const payload = {
            _claudeInstructions: {
                ...
                mandate: [
                    ...
                    "BACKGROUND: Ensure the presentation slides always have a white or light-colored background. Do NOT use dark mode, and do NOT use heavy background colors. Maintain high contrast for readability.",
                ...
                robotLabels: {
                    ...
                    synthesizer: { title: "Investment Board Verdict",   icon: "🏛️" },
...
// ═════════════════════════════════════════════════════════════════════
// Tool 4.3: SAVE-ARTIFACT
// ═════════════════════════════════════════════════════════════════════
server.tool(
    "save-artifact",
    "Save any arbitrary file (e.g. .html, .xlsx, .pptx, .md) generated by Claude into the product's assets directory. Supports both text and base64 encoded binary files.",
    {
        productSlug: z.string().describe("The product slug"),
        filename: z.string().describe("The name of the file to save, e.g. '2026-05-26-user-stories-viewer.html' or 'financial-model.xlsx'"),
        fileContent: z.string().describe("The full string content of the file. If encoding is base64, this must be the base64 string."),
        encoding: z.enum(["utf-8", "base64"]).default("utf-8").describe("The encoding of the fileContent. Use 'base64' for binary files like .xlsx or .pptx."),
    },
    async ({ productSlug, filename, fileContent, encoding }) => {
        const product = await productRegistry.get(productSlug);
        if (!product) {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Unknown product: ${productSlug}` }, null, 2) }]
            };
        }
        ...
        await fs.writeFile(filepath, buffer, encoding === "base64" ? null : "utf-8");
        
        return {
            content: [{ type: "text", text: `Success! Artifact saved to ${filepath}` }]
        };
    }
);
...
Phase 2 robots run after Phase 1 is complete and promote-to-phase-2 has been confirmed.`,
    ...
    async () => {
        const phase1Robots = [
            { phase: 1, key: "epic",      type: "epics",                 description: "Epic breakdown — distinct functional streams with clear business goals and customer value" },
            ...
            return {
                content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        actionRequired: "accept-terms",
                        message: "Profile saved. You must accept the Terms of Service before running analyses. Call 'accept-terms' to proceed. Full terms: docs/EULA.md",
                    }, null, 2)
                }]
            };
        }
...
// ═════════════════════════════════════════════════════════════════════
// Tool 11: EPIC-LIST — list all epics and features for a product
// ═════════════════════════════════════════════════════════════════════
server.tool(
    "epic-list",
    `List all epics generated for a given product by reading the epic robot output. Use this to determine valid epicId and featureId values to pass to downstream robots.`,
    {
        productSlug: z.string().describe("The product slug"),
    },
    async ({ productSlug }) => {
        ...
        const epicOutput = await teamLeader.assetStore.loadLatestRobotOutput(productSlug, "epic");
        if (!epicOutput) {
            return {
                content: [{
                    type: "text",
                    text: JSON.stringify({ error: "No epic output found. Run the 'epic' robot and save its output first." }, null, 2)
                }]
            };
        }

        // Try to parse the JSON block from the epic output
        ...
        if (jsonMatch) {
            ...
            if (braceStart !== -1 && braceEnd !== -1) {
                jsonStr = epicOutput.substring(braceStart, braceEnd + 1);
            }
        }
        ...
        try {
            ...
            return {
                content: [{
                    type: "text",
                    text: JSON.stringify({ error: "Failed to parse epic output JSON.", raw: epicOutput }, null, 2)
                }]
            };
        }

        // The exact structure depends on Claude's output based on epicStructure, but typically it should contain an array of epics.
        return {
            content: [{
                type: "text",
                text: JSON.stringify({ epics: parsed }, null, 2)
            }]
        };
    }
);
...
// ═════════════════════════════════════════════════════════════════════
// Tool 12: PRODUCT-CREATE — scaffold a new product
// ═════════════════════════════════════════════════════════════════════
server.tool(
    "product-create",
...
3. Call save-pdd with the JSON Claude produced`,
    {
        productSlug: z.string().describe("The product slug"),
        epicId: z.string().optional().describe("Optional epic ID to generate a scoped PDD for"),
    ...
    async ({ productSlug, epicId, pddVersion }) => {
        const product = await productRegistry.get(productSlug);
        if (!product) {
            return {
                content: [{
        ...
        let assemblyPayload;
        try {
            assemblyPayload = await pddComposer.assemble(productSlug, epicId);
        } catch (err) {
            return {
                content: [{
                    type: "text",
...
// ═════════════════════════════════════════════════════════════════════
// Tool: ACCEPT-TERMS — accept the ProductFlow EULA
// ═════════════════════════════════════════════════════════════════════
server.tool(
    "accept-terms",
    `Accept the ProductFlow Terms of Service / EULA.

Must be called once per persona before running any robot analyses. Until accepted,
'pm-profile-save' will return actionRequired: "accept-terms".

What the terms cover:
  - Usage data (ratings, feedback notes) stored locally only in data/brain-database.json
  - Anonymised process improvement suggestions optionally emailed to the owner
    (only if PRODUCTFLOW_OWNER_EMAIL + SMTP env vars are set — off by default)
  - No product names, ideas, or personal data are included in any notification

Full terms text: docs/EULA.md in the project root.`,
    {
        agreedToTerms: z.literal(true).describe("Must be explicitly true — passing false is rejected"),
        eulaVersion:   z.string().optional().describe("EULA version string, e.g. '1.0'. Defaults to '1.0'."),
    },
    async ({ agreedToTerms, eulaVersion }) => {
        const version = eulaVersion || "1.0";
        const saved   = await pmProfile.acceptEula(version);
        return {
            content: [{
                type: "text",
                text: JSON.stringify({
                    ...
                    eulaVersion: version,
                    profile:     saved,
                    message:     "Terms accepted. You may now run robot analyses.",
                }, null, 2)
            }]
        };
    }
...
// ═════════════════════════════════════════════════════════════════════
// Tool: GET-PROCESS-SUGGESTIONS — retrieve process improvement suggestions
// ═════════════════════════════════════════════════════════════════════
server.tool(
    "get-process-suggestions",
    `Retrieve process improvement suggestions generated by the Process Advisor.

Suggestions are created automatically after every 5 feedback events for a robot.
Two suggestion types:
  improve-robot  — robot's average rating has fallen below 3.5 across 5+ runs
  new-robot      — feedback notes repeatedly mention a topic no existing robot covers

Suggestions with frequency >= NOTIFICATION_THRESHOLD (default 3) are also emailed to
the owner when PRODUCTFLOW_OWNER_EMAIL is configured.`,
    {
        status: z.enum(["pending", "sent", "dismissed", "all"])
                  .optional()
                  .describe("Filter by status. Defaults to 'pending'."),
    },
    async ({ status = "pending" }) => {
        const all     = teamLeader.database.data.processImprovements || [];
        const results = status === "all" ? all : all.filter(s => s.status === status);
        return {
            content: [{
                type: "text",
                text: JSON.stringify({
                    ...
                    message:     results.length === 0
                        ? `No ${status === "all" ? "" : status + " "}process improvement suggestions yet.`
                        : `${results.length} suggestion(s) found. Review and share with the AutoPM maintainer if relevant.`,
                }, null, 2)
            }]
        };
    }
